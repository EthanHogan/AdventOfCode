...
function getSumOfSub100kSizeDirectories(commandStr: string): Number {
  ...
  // get directories that are less than 100K size
  const lessThan100KDirs = directories.filter((d) => d.size < 100000);
  // sum the less than 100k directories
  let sumOfLessThan100KDirs = 0;
  lessThan100KDirs.forEach((dir) => {
    sumOfLessThan100KDirs += dir.size;
  });
  return sumOfLessThan100KDirs;
}
...
console.log("Answer: ", getSumOfSub100kSizeDirectories(sampleInput));

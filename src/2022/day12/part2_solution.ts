...
function findFewestStepsToBestSignal(heightMapInput: string): number {
    ...
    const startingPositions: Elevation[] = getAllElevationsBySymbol("a", heightMap);
    let shortestPathLength = null;
    for(const startingPosition of startingPositions) {
        const pathsToEnd = findPathsFromStartToEnd({...startingPosition}, [...heightMap]).sort((a,b) => a.length - b.length);
        if (pathsToEnd.length === 0) continue;
        const shortestPathLengthFromStartingPosition = pathsToEnd[0].length - 1;
        if (shortestPathLength === null) {
            shortestPathLength = shortestPathLengthFromStartingPosition;
            continue;
        }
        if (shortestPathLengthFromStartingPosition < shortestPathLength) {
            shortestPathLength = shortestPathLengthFromStartingPosition;
        }
    }
...
function getAllElevationsBySymbol(symbol: string, heightMap: HeightMap): Elevation[] {
    const elevationsForSymbol: Elevation[] = [];
    for (let r = 0; r < heightMap.length; r++) {
        ...
        for (let c = 0; c < row.length; c++) {
            ...
            if (currentElevation.symbol === symbol) {
                elevationsForSymbol.push(currentElevation);
            }
        }
    }
    // if the return does happen, then the symbol is not it the list and this will return empty
    return elevationsForSymbol;
}

...
function getTopCrateOfEachStack(input: string): string {
  ...
  console.log("parsed result: ", parsedStacksAndProcedures);
  ...
  console.log("Final stacks: ", parsedStacksAndProcedures.stacks);
...
function executeProcedureOnStacks(
  ...
  // reverse the order because the crates are actually taken 1 at a time. Do the top box of the `from` stack will actually be the first crate added to the `to`.
...
function parseListOfStacks_FromStacksStr(stacksStr: string): Stack[] {
  ...
  const result: Stack[] = [];
  console.log("STACK NUMBER TUPLES:", stackNumberAndPositionTuples);
